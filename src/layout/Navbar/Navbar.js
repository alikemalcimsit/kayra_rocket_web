...
if(active===1){
 ...
 <div className=" md:visible invisible flex w-full h-20 justify-center items-center md:fixed md:z-20  md:bg-gray-400 ">
    <ul ref={ref}  className="flex md:gap-x-20 justify-center md:text-white   items-center     ">


      <Link  to="/" spy={true} smooth={true} offset={-150} duration={700} ><li className="rounded-3xl p-2" onClick={()=>setActive(1) }>Home</li> </Link>  
       <Link to="about" spy={true} smooth={true} offset={-100} duration={700}><li className="rounded-3xl p-2" onClick={()=>setActive(2) }>About</li></Link> 
       <Link to="members" spy={true} smooth={true} offset={-100} duration={700}><li className="rounded-3xl p-2" onClick={()=>setActive(3)}>Members</li></Link> 
        <Link to="contact" spy={true} smooth={true} offset={-150} duration={700}><li className="rounded-3xl p-2" onClick={()=>setActive(4) }>Contact</li></Link>
    </ul>
 </div>
  );
}
